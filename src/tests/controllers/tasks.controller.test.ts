...
test('updateTask', async t => {
    ...
    const updated_task = await request(app).put(`/task/${task_generated.body.task_id}`).send(task_to_update);
...
test('deleteTaskByID', async t => {
    ...
    console.log(task_generated.body);

    const task_to_delete = {
        task_id: task_generated.body.task_id
    }

    const deleted_task = await request(app).delete(`/task/${task_to_delete.task_id}`);
